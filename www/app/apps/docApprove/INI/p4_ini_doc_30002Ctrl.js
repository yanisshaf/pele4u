 ...
 */
angular.module('pele')
  .controller('p4_ini_doc_30002Ctrl', function($rootScope, $scope, $stateParams, $http, $q, $location, $window, $timeout, $ionicLoading, $ionicActionSheet, $ionicModal, PelApi, $ionicNavBarDelegate, $cordovaNetwork, $ionicPopup, appSettings, $sessionStorage, $cordovaFileTransfer, $cordovaInAppBrowser
      ...
      $scope.openAttachedFile = function(p_openFileName, p_fullFileName, p_fileType, p_fileMaofType, p_shortText, p_longText, isOpened, p_iosOpenfileName) {
        ...
        if ("Y" === isOpened) {
          ...
          retGetFileURI.success(function(data, status) {
            ...
            function(response) {
              PelApi.lagger.info("GetFileURI : " + JSON.stringify(response));
              $ionicLoading.hide();
              $scope.$broadcast('scroll.refreshComplete');
      ...
      $scope.doRefresh = function() {
        ...
        $scope.INITIATED_DETAILS_CUR = appSettings.config.INITIATED_DETAILS_CUR[0];
        ...
        $scope.INI_ACTION_HISTORY = $scope.addPushFlagToActionHistory(appSettings.config.docDetails.INITIATED_APPROVAL_LIST_CUR);
        $scope.INI_ATTACHED_DOCUMENTS = $scope.getAttachedDocuments(appSettings.config.docDetails.INITIATED_ATTACHMENTS_CUR);
